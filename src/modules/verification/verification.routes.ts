...
import {
    submitAadhaarSchema, submitAadhaarOtpSchema, submitGovtIdSchema,
...
// Seller
router.post("/aadhaar/generate-otp", protect, sellerLimiter, resolveTenant, requireSellerRole("owner"),
    validate(submitAadhaarSchema), verificationController.requestAadhaarOtp);

router.post("/aadhaar/verify-otp", protect, sellerLimiter, resolveTenant, requireSellerRole("owner"),
    validate(submitAadhaarOtpSchema), verificationController.confirmAadhaarOtp);
...
router.patch("/:sellerId/aadhaar/verify", protect, sellerLimiter,
    requirePlatformAdmin("super_admin", "onboarding_manager"), validate(sellerParamSchema),
    verificationController.verifyAadhaar);

router.patch("/:sellerId/aadhaar/reject", protect, sellerLimiter,
    requirePlatformAdmin("super_admin", "onboarding_manager"), validate(rejectVerificationSchema),
